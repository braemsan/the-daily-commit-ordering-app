...
export default function App() {
  ...
  const canSubmit = name.trim().length > 0 && itemCount > 0 && !submitting
            ...
            {menu.map((item) => (
              <article className="menu-card" key={item.id}>
                <div>
                  <h2>{item.name}</h2>
                  <p className="price">
                    {item.price === null ? 'Pay at booth' : `$${item.price.toFixed(2)}`}
                  </p>
                </div>
                ...
                {item.sugar_option ? (
                    ...
                    {(['sugar', 'no_sugar'] as const).map((option) => {
                      ...
                      return (
                        <div className="quantity-row" key={option}>
                          ...
                          <QuantityControl
                            quantity={quantity}
                            onMinus={() => adjust(item, -1, option)}
                            onPlus={() => adjust(item, 1, option)}
                          />
                        </div>
                      )
                    })}
                  ...
                  <QuantityControl
                    quantity={cart[keyFor(item)]?.quantity ?? 0}
                    onMinus={() => adjust(item, -1)}
                    onPlus={() => adjust(item, 1)}
                  />
                )}
              </article>
            ))}
          ...
          <section className="checkout-card">
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="e.g. less ice, takeaway"
                maxLength={200}
              />
            </label>
